...
(function (global) {
  ...
  const ASSET = `assets/buffering.gif?v=${encodeURIComponent(global.EOD_APP_VERSION || '3.4.28')}`;
  const SKIP_RE = /sas-auth-status|rebotics-auth-status|\/api\/sas-user\/|\/usage\b|eod-version\.json|\/api\/me(?:\?|$)|digital-signoffs\/(?:heartbeat|catalog-stores|sync|visit-mirror)|\/photos\/|\/image(?:\?|$)|field-set\/(?:status|planogram|planogram-image|photo|jobs)|\/api\/shifts\/day|sas-upload/i;

...
(function (global) {
  ...
  const ASSET = `assets/buffering.gif?v=${encodeURIComponent(global.EOD_APP_VERSION || '3.3.10')}`;
  const SKIP_RE = /sas-auth-status|rebotics-auth-status|\/usage\b|eod-version\.json|\/api\/me(?:\?|$)|digital-signoffs\/(?:sync|heartbeat)|\/photos\/|\/image(?:\?|$)|field-set\/(?:status|planogram)/i;

/* Compass buffering overlay Ã¢â‚¬â€ stages + spinner until Success. */
(function (global) {
  'use strict';
  ...
  const ASSET = `assets/buffering.gif?v=${encodeURIComponent(global.EOD_APP_VERSION || '3.4.46')}`;
  const SKIP_RE = /sas-auth-status|rebotics-auth-status|\/api\/sas-user\/|\/usage\b|eod-version\.json|\/api\/me(?:\?|$)|digital-signoffs\/(?:heartbeat|catalog-stores|sync|visit-mirror)|field-session\/|\/photos\/|\/image(?:\?|$)|field-set\/(?:status|planogram|planogram-image|photo|jobs|reconcile)|\/api\/shifts\/day|sas-upload/i;

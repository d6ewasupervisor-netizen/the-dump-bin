...
function runDeterministic(name: string, cmd: string, args: string[]): { ok: boolean; detail: string } {
...
async function runAgentScenario(scenario: Scenario): Promise<{ ok: boolean; detail: string }> {
...
async function main(): Promise<number> {
  const results: { scenario: string; ok: boolean; detail: string; skipped?: boolean }[] = [];
  ...
  for (const [name, cmd, args] of [
    ...
    if (!r.ok) console.log(r.detail.slice(0, 400));
  }
